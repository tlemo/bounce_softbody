...
b3BodyTriangleShape::b3BodyTriangleShape(const b3BodyTriangleShapeDef& def, b3Body* body) : b3BodyShape(def, body)
{
	m_type = e_bodyTriangleShape;
...
b3AABB b3BodyTriangleShape::ComputeAABB() const
{
	b3AABB aabb;
...
void b3BodyTriangleShape::Synchronize(const b3Vec3& displacement)
{
	b3AABB aabb = ComputeAABB();
	m_body->m_tree.MoveProxy(m_proxyId, aabb, displacement);
}

bool b3BodyTriangleShape::RayCast(b3RayCastOutput* output, const b3RayCastInput& input) const
{
	b3Vec3 p1 = input.p1;
	b3Vec3 p2 = input.p2;
