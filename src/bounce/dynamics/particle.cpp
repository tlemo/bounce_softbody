...
void b3Particle::DestroyShapes()
{
	{
		// Destroy spheres
		b3BodySphereShape* s = m_body->m_sphereShapeList.m_head;
		while (s)
		{
			b3BodySphereShape* s0 = s;
			s = s->m_next;
	...
	{
		// Destroy triangles
		b3BodyTriangleShape* t = m_body->m_triangleShapeList.m_head;
		while (t)
		{
			b3BodyTriangleShape* t0 = t;
			t = t->m_next;
...
void b3Particle::SynchronizeShapes()
{
	// Synchronize triangles
	for (b3BodyTriangleShape* t = m_body->m_triangleShapeList.m_head; t; t = t->m_next)
	{
