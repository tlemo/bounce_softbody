...
#ifndef UNIFORM_SOFTBODY_H
#define UNIFORM_SOFTBODY_H

#include <bounce/bounce_softbody.h>

struct SoftBodyMeshTriangle
{
	int v1, v2, v3;
};

struct SoftBodyMeshTetrahedron
{
	int v1, v2, v3, v4;
};
...
struct SoftBodyMesh
{
	...
	b3Vec3* vertices;
	int triangleCount;
	SoftBodyMeshTriangle* triangles;
	int tetrahedronCount;
	SoftBodyMeshTetrahedron* tetrahedrons;
	...
	SoftBodyMeshTriangle GetTriangle(int index) const
	{
		return triangles[index];
	}

	SoftBodyMeshTetrahedron GetTetrahedron(int index) const
	{
...
// b3SoftBody wrapper. 
class UniformSoftBody : public b3SoftBody
{
public:
	UniformSoftBody();
	UniformSoftBody(const ClothDef& def);
	UniformSoftBody(const TetDef& def);
	~UniformSoftBody();

	// Return particle associated with given mesh vertex.
	b3SoftBodyParticle* GetParticle(int index)
	{
		assert(index < m_mesh->vertexCount);
		return m_particles[index];
	}
private:
	const SoftBodyMesh* m_mesh;
	b3SoftBodyParticle** m_particles;
};

template<int H = 1, int W = 1>
struct GridClothMesh : public SoftBodyMesh
{
	b3Vec3 gridVertices[(H + 1) * (W + 1)];
	SoftBodyMeshTriangle gridTriangles[2 * H * W];
	...
	GridClothMesh()
	{
		...
		for (int i = 0; i < H; ++i)
		{
			for (int j = 0; j < W; ++j)
			{
				...
				SoftBodyMeshTriangle* t1 = triangles + triangleCount++;
				t1->v1 = v1;
				t1->v2 = v2;
				t1->v3 = v3;

				SoftBodyMeshTriangle* t2 = triangles + triangleCount++;
				t2->v1 = v3;
				t2->v2 = v4;
				t2->v3 = v1;
			}
...
struct GridTetMesh : public SoftBodyMesh
{
	b3Vec3 gridVertices[(H + 1) * (W + 1) * (D + 1)];
	SoftBodyMeshTriangle gridTriangles[4 * H * W + 4 * H * D + 4 * W * D];
	SoftBodyMeshTetrahedron gridTetrahedrons[5 * H * W * D];
